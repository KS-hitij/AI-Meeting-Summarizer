...
def decide_text_or_audio(state: AgentState):
    ...
    if not state.get("file"):
        raise ValueError(
            "No file found in agent state. Expected file with `file_type`."
        )
    ...
    if file_type == "audio":
        ...
        raise ValueError(
            f"Unsupported file type: {file_type!r}. " "Expected 'audio' or 'text'."
        )
...
def summarize_node(state: AgentState):
    ...
    response = summarizing_model_with_structured_output.invoke(
        [summarizer_system_msg, HumanMessage(content=prompt)]
    )
...
graph.add_conditional_edges(
    START, decide_text_or_audio, {"transcribe": "transcribe", "summarize": "summarize"}
)
